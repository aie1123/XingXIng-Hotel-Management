...
namespace StudentWeb.DAL
{
	...
	public partial class RoomInsepctionDataDAO : IRoomInsepctionDataDAO
	{
        ...
        public PagedList<dynamic> GetPagingRoomInsepctionList(int pageIndex, int pageSize, string whereClause = null)
        {
            pageIndex--;
            string where = $@"where RoomInsepctionData.RoominsepctionID > 0 {whereClause}";
            string orderBy = "RoomInsepctionData.RoominsepctionID DESC";
            var startItem = pageIndex * pageSize + 1;
            var endItem = (pageIndex + 1) * pageSize;
            var selFields = $@"RoominsepctionID, RoomNumber, Satisfaction, Problems";
            var tabSql = $" RoomInsepctionData  {where}";
            var sql = $@"SELECT COUNT(RoomInsepctionData.RoominsepctionID) FROM {tabSql}
SELECT *
