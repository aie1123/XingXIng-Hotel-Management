...
namespace StudentWeb.DAL
{
    ...
    public partial class GuestDataDAO : IGuestDataDAO
    {
        ...
        public PagedList<dynamic> GetPagingGuestList(int pageIndex, int pageSize, string whereClause = null)
        {
            pageIndex--;
            string where = $@"where GuestData.GuestID > 0 {whereClause}";
            string orderBy = "GuestData.GuestID DESC";
            var startItem = pageIndex * pageSize + 1;
            var endItem = (pageIndex + 1) * pageSize;
            var selFields = $@"GuestID,GuestPPID, GuestName, GuestGender, GuestPhone, GuestRemarks";
            var tabSql = $" GuestData  {where}";
            var sql = $@"SELECT COUNT(GuestData.GuestID) FROM {tabSql}
SELECT *
