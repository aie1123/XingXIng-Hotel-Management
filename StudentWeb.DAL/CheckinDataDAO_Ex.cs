...
namespace StudentWeb.DAL
{
    ...
    public partial class CheckinDataDAO : ICheckinDataDAO
    {
        ...
        public PagedList<dynamic> GetPagingCheckinList(int pageIndex, int pageSize, string whereClause = null)
        {
            pageIndex--;
            string where = $@"where CheckinData.CheckinID > 0 {whereClause}";
            string orderBy = "CheckinData.CheckinID DESC";
            var startItem = pageIndex * pageSize + 1;
            var endItem = (pageIndex + 1) * pageSize;
            var selFields = $@"CheckinID, RoomNumber, CheckinDate, LeaveDate, DayCount, RoomID, RoomState, OrderID";
            var tabSql = $" CheckinData  {where}";
            var sql = $@"SELECT COUNT(CheckinData.CheckinID) FROM {tabSql}
SELECT *
